/**
 * Sidebar de navigation pour le module Projets-en-Cours
 */
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, Briefcase } from 'lucide-react';
import { projetsNavigationConfig, type NavNode } from './projetsNavigationConfig';

interface ProjetsSidebarProps {
...
export function ProjetsSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'delayed' || node.id === 'critical' || node.id === 'at-risk') return 'critical';
      if (node.id === 'active' || node.id === 'high' || node.id === 'blocked') return 'warning';
    }
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(projetsNavigationConfig).map((node) => {
            const Icon = node.icon;
