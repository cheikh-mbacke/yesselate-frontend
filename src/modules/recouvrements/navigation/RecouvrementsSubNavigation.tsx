/**
 * Navigation secondaire et tertiaire pour le module Recouvrements
 */
...
import {
  recouvrementsNavigationConfig,
  getSubCategories,
  getSubSubCategories,
  type NavNode,
} from './recouvrementsNavigationConfig';
import type { RecouvrementsMainCategory } from '../types/recouvrementsTypes';

interface RecouvrementsSubNavigationProps {
...
export function RecouvrementsSubNavigation({
  mainCategory,
  subCategory,
  subSubCategory,
  onSubCategoryChange,
  ...
  stats = {},
}: RecouvrementsSubNavigationProps) {
  const subCategories = getSubCategories(mainCategory);
  const subSubCategories = subCategory ? getSubSubCategories(mainCategory, subCategory) : [];

  const mainLabel = recouvrementsNavigationConfig[mainCategory]?.label || 'Recouvrements';
  ...
  return (
    <div className="bg-slate-900/60 border-b border-slate-700/50">
      <div className="px-4 py-2 flex items-center gap-2 text-sm border-b border-slate-800/50">
        <span className="text-slate-500">Recouvrements</span>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-cyan-500/15 text-slate-200 border border-cyan-500/30 scale-105'
