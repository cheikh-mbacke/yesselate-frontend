/**
 * Navigation secondaire et tertiaire pour le module Litiges
 */
...
import {
  litigesNavigationConfig,
  getSubCategories,
  getSubSubCategories,
  type NavNode,
} from './litigesNavigationConfig';
import type { LitigesMainCategory } from '../types/litigesTypes';

interface LitigesSubNavigationProps {
...
export function LitigesSubNavigation({
  mainCategory,
  subCategory,
  subSubCategory,
  onSubCategoryChange,
  ...
  stats = {},
}: LitigesSubNavigationProps) {
  const subCategories = getSubCategories(mainCategory);
  const subSubCategories = subCategory ? getSubSubCategories(mainCategory, subCategory) : [];

  const mainLabel = litigesNavigationConfig[mainCategory]?.label || 'Litiges';
  ...
  return (
    <div className="bg-slate-900/60 border-b border-slate-700/50">
      <div className="px-4 py-2 flex items-center gap-2 text-sm border-b border-slate-800/50">
        <span className="text-slate-500">Litiges</span>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-rose-500/15 text-slate-200 border border-rose-500/30 scale-105'
