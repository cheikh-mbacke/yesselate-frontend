 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Imports par le nom du module (ex: Delegations, Finances)
...
interface ModalState {
  isOpen: boolean;
  type: ImportsModalType | null;
  data?: any;
}

interface ImportsModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function ImportsModals({ modal, onClose }: ImportsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <ImportsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <ImportsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <ImportsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <ImportsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <ImportsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <ImportsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function ImportsStatsModal({ onClose }: { onClose: () => void }) {
...
// Detail Modal
// ================================
function ImportsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
...
// Export Modal
// ================================
function ImportsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function ImportsSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function ImportsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function ImportsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
