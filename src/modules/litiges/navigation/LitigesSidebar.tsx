/**
 * Sidebar de navigation pour le module Litiges
 */
...
import { litigesNavigationConfig, type NavNode } from './litigesNavigationConfig';

interface LitigesSidebarProps {
...
export function LitigesSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'critical' || node.id === 'active' || node.id === 'high') return 'critical';
      if (node.id === 'pending' || node.id === 'old') return 'warning';
    }
  ...
  const NavNodeComponent = React.memo(function NavNodeComponent({
    ...
    return (
      <div className={cn('w-full', level > 0 && 'ml-4')}>
        ...
        >
          {isActive && level === 0 && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 rounded-r-full bg-rose-400" />
          )}
          {hasChildren && (
          ...
          {Icon && (
            <Icon className={cn('h-4 w-4 flex-shrink-0', isActive ? 'text-rose-400' : 'text-slate-400')} />
          )}
          {!collapsed && (
            <>
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(litigesNavigationConfig).map((node) => {
            const Icon = node.icon;
      ...
      <div className="p-4 border-b border-slate-800">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Litiges</h2>
          </div>
          <Button
            variant="ghost"
            size="sm"
