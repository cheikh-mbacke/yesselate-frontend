/**
 * Navigation secondaire et tertiaire pour le module IA
 */
...
import { iaNavigationConfig, getSubCategories, getSubSubCategories, type NavNode } from './iaNavigationConfig';
import type { IAMainCategory } from '../types/iaNavigationTypes';

interface IASubNavigationProps {
...
export function IASubNavigation({ mainCategory, subCategory, subSubCategory, onSubCategoryChange, onSubSubCategoryChange, stats = {} }: IASubNavigationProps) {
  const subCategories = getSubCategories(mainCategory);
  const subSubCategories = subCategory ? getSubSubCategories(mainCategory, subCategory) : [];
  const mainLabel = iaNavigationConfig[mainCategory]?.label || 'IA';
            ...
            {subCategories.map((subCat) => {
              ...
              return (
                <button key={subCat.id} onClick={() => onSubCategoryChange(subCat.id)} className={cn('px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-colors', isActive ? 'bg-blue-500/20 text-blue-300 border border-blue-500/30' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50 border border-transparent')}>
                  {subCat.label}
                  {badge !== undefined && badge !== null && badge !== 0 && <Badge variant={badgeType === 'critical' ? 'urgent' : badgeType === 'warning' ? 'warning' : badgeType === 'success' ? 'success' : 'default'} className="ml-2 h-5 min-w-5 px-1.5 text-xs">{badge}</Badge>}
                </button>
              );
            })}
          </div>
            ...
            {subSubCategories.map((subSubCat) => {
              ...
              return (
                <button key={subSubCat.id} onClick={() => onSubSubCategoryChange?.(subSubCat.id)} className={cn('px-2.5 py-1 rounded-md text-xs font-medium whitespace-nowrap transition-colors', isActive ? 'bg-blue-500/15 text-blue-300 border border-blue-500/20' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800/30 border border-transparent')}>
                  {subSubCat.label}
                  {badge !== undefined && badge !== null && badge !== 0 && <Badge variant={badgeType === 'critical' ? 'urgent' : badgeType === 'warning' ? 'warning' : badgeType === 'success' ? 'success' : 'default'} className="ml-1.5 h-4 min-w-4 px-1 text-xs">{badge}</Badge>}
                </button>
              );
            })}
          </div>
