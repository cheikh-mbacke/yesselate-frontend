/**
 * Sidebar de navigation pour le module Alerts
...
import { alertsNavigationConfig, type NavNode } from './alertsNavigationConfig';
...
interface AlertsSidebarProps {
  ...
  onCategoryChange: (category: AlertsMainCategory, subCategory?: string) => void;
...
export function AlertsSidebar({
  ...
  const getBadgeTypeForNode = useCallback(
    (node: NavNode): 'default' | 'warning' | 'critical' | 'success' => {
      ...
      if (badge && typeof badge === 'number' && badge > 0) {
        ...
        if (node.id === 'projets' || node.id.includes('projet')) return 'warning';
      }
  ...
  const handleNodeClick = useCallback(
    (category: AlertsMainCategory, subCategory?: string) => {
  ...
  const isNodeActive = useCallback(
    (nodeId: string): boolean => {
      if (nodeId === activeCategory) {
        if (!activeSubCategory) return true;
        const node = alertsNavigationConfig[nodeId as AlertsMainCategory];
        return node?.children?.some((child) => child.id === activeSubCategory) || false;
      }
      return false;
    },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        ...
        {collapsed && <AlertTriangle className="h-5 w-5 text-amber-400 mx-auto" />}
        {onToggleCollapse && (
          <Button
            variant="ghost"
            size="sm"
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {Object.entries(alertsNavigationConfig).map(([key, node]) => {
              ...
              <div key={key}>
                <button
                  onClick={() => {
                    ...
                    handleNodeClick(key as AlertsMainCategory);
                  }}
                  className={cn(
                    ...
                    {node.children?.map((subNode) => {
                      ...
                      return (
                        <button
                          key={subNode.id}
                          onClick={() =>
                            handleNodeClick(key as AlertsMainCategory, subNode.id)
                          }
                          className={cn(
