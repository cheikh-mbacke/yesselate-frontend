/**
 * Paiements Notification Panel
 * Panneau slide-in pour les notifications et alertes
 */
...
export function PaiementsNotificationPanel({ isOpen, onClose }: PaiementsNotificationPanelProps) {
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      id: '1',
      type: 'urgent',
      title: 'Paiement critique',
      message: 'PAY-2024-001 nécessite une validation urgente (échéance dans 2 jours)',
    ...
    },
    {
      ...
      type: 'warning',
      title: 'SLA proche',
      message: '5 paiements approchent de leur SLA (< 24h)',
    ...
    },
    {
      ...
      message: 'PAY-2024-015 a été validé avec succès',
    ...
    },
    {
      ...
      message: 'PAY-2024-032 ajouté à la file d\'attente',
      ...
      <div className="fixed right-0 top-0 bottom-0 w-96 bg-slate-900 border-l border-slate-700/50 shadow-2xl z-50 flex flex-col animate-slideInRight">
        ...
        <div className="px-4 py-3 border-t border-slate-800 bg-slate-900/80">
          <div className="flex items-center justify-between text-xs text-slate-500">
            ...
            {unreadCount > 0 && (
              <span className="text-emerald-400">{unreadCount} non lu{unreadCount > 1 ? 'es' : 'e'}</span>
            )}
          </div>
        </div>
      </div>
