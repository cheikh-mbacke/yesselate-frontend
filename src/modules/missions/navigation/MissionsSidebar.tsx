/**
 * Sidebar de navigation pour le module Missions
 */
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, Target } from 'lucide-react';
import { missionsNavigationConfig, type NavNode } from './missionsNavigationConfig';
  ...
  const NavNodeComponent = React.memo(function NavNodeComponent({
    ...
    return (
      <div className={cn('w-full', level > 0 && 'ml-4')}>
        ...
        >
          {isActive && level === 0 && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 rounded-r-full bg-violet-400" />
          )}
          {hasChildren && (
          ...
          {Icon && (
            <Icon className={cn('h-4 w-4 flex-shrink-0', isActive ? 'text-violet-400' : 'text-slate-400')} />
          )}
          {!collapsed && (
            <>
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(missionsNavigationConfig).map((node) => {
            const Icon = node.icon;
