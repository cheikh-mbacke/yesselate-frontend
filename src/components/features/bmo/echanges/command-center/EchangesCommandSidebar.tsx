/**
 * Sidebar de navigation principale pour Échanges Inter-Bureaux
...
const echangesCategories: SidebarCategory[] = [
  { id: 'overview', label: "Vue d'ensemble", icon: LayoutDashboard },
  { id: 'inbox', label: 'Boîte de réception', icon: Inbox, badge: 24 },
  { id: 'urgent', label: 'Urgents', icon: Zap, badge: 5, badgeType: 'warning' },
  { id: 'escalated', label: 'Escaladés', icon: ArrowUp, badge: 3, badgeType: 'critical' },
  { id: 'pending', label: 'En attente', icon: Clock, badge: 12 },
  { id: 'resolved', label: 'Résolus', icon: CheckCircle, badge: 142 },
  { id: 'by-bureau', label: 'Par bureau', icon: Building2 },
  { id: 'analytics', label: 'Analytics', icon: BarChart3 },
  { id: 'history', label: 'Historique', icon: History },
];

interface EchangesCommandSidebarProps {
...
export const EchangesCommandSidebar = React.memo(function EchangesCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: EchangesCommandSidebarProps) {
  return (
    <aside
      className={cn(
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        ...
        {collapsed && (
          <MessageSquare className="h-5 w-5 text-violet-400 mx-auto" />
        )}
        <Button
          variant="ghost"
          size="sm"
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {echangesCategories.map((category) => {
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  'group relative',
                  isActive
                    ? 'bg-violet-500/10 border border-violet-500/30 scale-[1.02]'
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isActive ? 'text-violet-400 scale-110' : 'text-slate-400 group-hover:text-slate-200 group-hover:scale-105'
                  )}
                >
