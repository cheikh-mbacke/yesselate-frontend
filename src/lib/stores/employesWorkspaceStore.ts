/**
 * STORE: Employes Workspace - Pattern Pilotage
 */
import { create } from 'zustand';
import { persist } from 'zustand/middleware';

export type EmployesTabType = 'inbox' | 'detail' | 'spof' | 'absences' | 'evaluations' | 'analytics';

export interface EmployesTab { id: string; type: EmployesTabType; title: string; icon: string; data: Record<string, unknown>; closable?: boolean; }
export interface EmployesFilter { status?: string; bureau?: string; contrat?: string; spof?: boolean; search?: string; }

interface EmployesWorkspaceState {
  tabs: EmployesTab[]; activeTabId: string | null; selectedIds: Set<string>; currentFilter: EmployesFilter;
  ...
  openTab: (tab: Omit<EmployesTab, 'id'> & { id?: string }) => void; closeTab: (tabId: string) => void; setActiveTab: (tabId: string) => void;
  toggleSelected: (id: string) => void; selectAll: (ids: string[]) => void; clearSelection: () => void;
  setFilter: (filter: Partial<EmployesFilter>) => void; clearFilter: () => void;
...
const defaultTabs: EmployesTab[] = [{ id: 'inbox:all', type: 'inbox', title: 'Tous les employés', icon: '👥', data: { queue: 'all' }, closable: false }];

export const useEmployesWorkspaceStore = create<EmployesWorkspaceState>()(
  persist((set, get) => ({
  ...
  }), { name: 'employes:workspace', partialize: (s) => ({ watchlist: s.watchlist, viewMode: s.viewMode }) })
);
