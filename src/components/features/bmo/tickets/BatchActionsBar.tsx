/**
 * Barre d'actions groupées pour Tickets
 */
...
interface BatchActionsBarProps {
  ...
  onClose?: () => void;
  onAssign?: () => void;
  onChangePriority?: (priority: 'critical' | 'high' | 'medium' | 'low') => void;
  onAddComment?: () => void;
  onEscalate?: () => void;
  onSendUpdate?: () => void;
  onExport?: () => void;
    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-4">
      <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          {onResolve && (
            <FluentButton variant="ghost" size="sm" onClick={onResolve} className="hover:bg-emerald-500/10 hover:text-emerald-400">
          ...
          {onClose && (
            <FluentButton variant="ghost" size="sm" onClick={onClose} className="hover:bg-slate-600/10 hover:text-slate-400">
          ...
          {onChangePriority && (
            <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-700/30 border border-slate-700">
              ...
              <button
                onClick={() => onChangePriority('critical')}
          ...
          {onEscalate && (
            <FluentButton variant="ghost" size="sm" onClick={onEscalate} className="hover:bg-rose-500/10 hover:text-rose-400">
          ...
          {onSendUpdate && (
            <FluentButton variant="ghost" size="sm" onClick={onSendUpdate} className="hover:bg-blue-500/10 hover:text-blue-400">
          ...
          {onAddTags && (
            <FluentButton variant="ghost" size="sm" onClick={onAddTags} className="hover:bg-purple-500/10 hover:text-purple-400">
