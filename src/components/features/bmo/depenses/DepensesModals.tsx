 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Depenses par le nom du module (ex: Delegations, Finances)
...
interface ModalState {
  isOpen: boolean;
  type: DepensesModalType | null;
  data?: any;
}

interface DepensesModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function DepensesModals({ modal, onClose }: DepensesModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <DepensesStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <DepensesDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <DepensesExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <DepensesSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <DepensesShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <DepensesConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function DepensesStatsModal({ onClose }: { onClose: () => void }) {
...
// Detail Modal
// ================================
function DepensesDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
...
// Export Modal
// ================================
function DepensesExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function DepensesSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function DepensesShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function DepensesConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
