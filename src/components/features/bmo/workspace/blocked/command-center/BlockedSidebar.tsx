/**
 * Sidebar de navigation principale du Blocked Command Center
 * Architecture harmonisée avec Analytics - Style Command Center
 */
...
import { useBlockedCommandCenterStore, type BlockedMainCategory } from '@/lib/stores/blockedCommandCenterStore';

interface SidebarCategory {
  id: BlockedMainCategory;
  label: string;
  icon: LucideIcon;
  badge?: number | string;
...
const blockedCategories: SidebarCategory[] = [
  { id: 'overview', label: 'Vue d\'ensemble', icon: LayoutGrid },
  { id: 'queue', label: 'Files d\'attente', icon: FileText },
  { id: 'critical', label: 'Critiques', icon: AlertCircle, badgeType: 'critical' },
  { id: 'matrix', label: 'Matrice urgence', icon: LayoutGrid },
  { id: 'bureaux', label: 'Par bureau', icon: Building2 },
  { id: 'timeline', label: 'Timeline', icon: History },
  { id: 'decisions', label: 'Décisions', icon: Zap, badgeType: 'warning' },
  { id: 'audit', label: 'Audit', icon: Shield },
];

interface BlockedCommandSidebarProps {
  activeCategory: BlockedMainCategory;
  collapsed: boolean;
  onCategoryChange: (category: BlockedMainCategory) => void;
  onToggleCollapse: () => void;
  onOpenCommandPalette: () => void;
}

export const BlockedCommandSidebar = React.memo(function BlockedCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: BlockedCommandSidebarProps) {
  const { stats } = useBlockedCommandCenterStore();

  // Compute badges dynamically
  const categoriesWithBadges = React.useMemo(() => {
    return blockedCategories.map((category) => {
      let badge: number | string | undefined;
      let badgeType: 'default' | 'warning' | 'critical' | undefined = category.badgeType;
      ...
      switch (category.id) {
        ...
        case 'critical':
          ...
          badgeType = stats?.critical && stats.critical > 0 ? 'critical' : undefined;
          break;
        case 'decisions':
          badge = stats?.escalatedToday ?? 0;
          badgeType = badge > 0 ? 'warning' : undefined;
          break;
        default:
          badge = category.badge;
      }

      return { ...category, badge, badgeType };
    });
  }, [stats]);
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Blocages</span>
          </div>
        )}
        {collapsed && (
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isActive ? 'text-red-400 scale-110' : 'text-slate-400 group-hover:text-slate-200 group-hover:scale-105'
                  )}
                >
