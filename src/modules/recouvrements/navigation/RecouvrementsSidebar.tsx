/**
 * Sidebar de navigation pour le module Recouvrements
 */
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, ArrowDownUp } from 'lucide-react';
import { recouvrementsNavigationConfig, type NavNode } from './recouvrementsNavigationConfig';

interface RecouvrementsSidebarProps {
...
export function RecouvrementsSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'failed' || node.id === 'overdue' || node.id === 'critical') return 'critical';
      if (node.id === 'pending' || node.id === 'at-risk') return 'warning';
    }
  ...
  const NavNodeComponent = React.memo(function NavNodeComponent({
    ...
    return (
      <div className={cn('w-full', level > 0 && 'ml-4')}>
        ...
        >
          {isActive && level === 0 && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 rounded-r-full bg-cyan-400" />
          )}
          {hasChildren && (
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(recouvrementsNavigationConfig).map((node) => {
            const Icon = node.icon;
      ...
      <div className="p-4 border-b border-slate-800">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Recouvrements</h2>
          </div>
          <Button
            variant="ghost"
            size="sm"
