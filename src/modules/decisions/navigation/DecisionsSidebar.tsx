/**
 * Sidebar de navigation pour le module Decisions
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, Gavel } from 'lucide-react';
import { decisionsNavigationConfig, type NavNode } from './decisionsNavigationConfig';
import type { DecisionsMainCategory } from '../types/decisionsNavigationTypes';

interface DecisionsSidebarProps {
  activeCategory: DecisionsMainCategory;
...
export function DecisionsSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
    ...
    const handleClick = () => {
      if (hasChildren) {
        ...
        if (level === 0 && node.children && node.children.length > 0) {
          onCategoryChange(node.id as DecisionsMainCategory, node.children[0].id);
        }
      } else {
        if (level === 1) onCategoryChange(activeCategory, node.id);
        else onCategoryChange(node.id as DecisionsMainCategory);
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        ...
        {collapsed && (
          <Gavel className="h-5 w-5 text-purple-400 mx-auto" />
        )}
        {onToggleCollapse && (
          <Button
            variant="ghost"
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {Object.values(decisionsNavigationConfig).map((node) => (
