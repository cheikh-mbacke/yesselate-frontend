 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Sinistres par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer rose par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type SinistresModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: SinistresModalType | null;
  data?: any;
}

interface SinistresModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function SinistresModals({ modal, onClose }: SinistresModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <SinistresStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <SinistresDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <SinistresExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <SinistresSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <SinistresShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <SinistresConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function SinistresStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-rose-400" />
...
// Detail Modal
// ================================
function SinistresDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-rose-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-rose-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-rose-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-rose-500 hover:bg-rose-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function SinistresExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function SinistresSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function SinistresShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function SinistresConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
