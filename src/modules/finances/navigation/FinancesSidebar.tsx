/**
 * Sidebar de navigation pour le module Finances
 */
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, Wallet } from 'lucide-react';
import { financesNavigationConfig, type NavNode } from './financesNavigationConfig';
...
export function FinancesSidebar({
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'overdue' || node.id === 'exceeded' || node.id === 'failed') return 'critical';
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(financesNavigationConfig).map((node) => {
            const Icon = node.icon;
