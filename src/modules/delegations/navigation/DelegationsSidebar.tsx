/**
 * Sidebar de navigation pour le module Delegations
 */
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, UserCheck } from 'lucide-react';
import { delegationsNavigationConfig, type NavNode } from './delegationsNavigationConfig';

interface DelegationsSidebarProps {
...
export function DelegationsSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'expired' || node.id === 'critical' || node.id === 'high') return 'critical';
      if (node.id === 'active' || node.id === 'pending') return 'warning';
    }
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(delegationsNavigationConfig).map((node) => {
            const Icon = node.icon;
      ...
      <div className="p-4 border-b border-slate-800">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Délégations</h2>
          </div>
          <Button
            variant="ghost"
            size="sm"
