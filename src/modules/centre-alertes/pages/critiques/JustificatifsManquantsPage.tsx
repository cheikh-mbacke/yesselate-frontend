/**
 * Page : Alertes critiques > Justificatifs manquants
 */
...
export function JustificatifsManquantsPage() {
  const { data: alertes, isLoading } = useAlertesData({
    typologies: ['CRITIQUE'],
  });

  const justificatifs = alertes?.filter((a) => a.titre.toLowerCase().includes('justificatif')) || [];
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-slate-200 mb-1">Justificatifs manquants</h2>
        <p className="text-sm text-slate-400">
          Liste des justificatifs manquants nécessitant une action
        </p>
      </div>

      <div className="space-y-3">
        {justificatifs.map((alerte) => (
          <div
            key={alerte.id}
            className="p-4 rounded-lg border border-red-500/30 bg-red-500/10"
          >
            <div className="flex items-start gap-3">
              <FileText className="h-5 w-5 text-red-400 mt-0.5" />
