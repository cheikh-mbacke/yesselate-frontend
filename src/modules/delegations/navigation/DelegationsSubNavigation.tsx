/**
 * Navigation secondaire et tertiaire pour le module Delegations
 */
...
import {
  delegationsNavigationConfig,
  getSubCategories,
  getSubSubCategories,
  type NavNode,
} from './delegationsNavigationConfig';
import type { DelegationsMainCategory } from '../types/delegationsTypes';

interface DelegationsSubNavigationProps {
...
export function DelegationsSubNavigation({
  mainCategory,
  subCategory,
  subSubCategory,
  onSubCategoryChange,
  ...
  stats = {},
}: DelegationsSubNavigationProps) {
  const subCategories = getSubCategories(mainCategory);
  const subSubCategories = subCategory ? getSubSubCategories(mainCategory, subCategory) : [];

  const mainLabel = delegationsNavigationConfig[mainCategory]?.label || 'Délégations';
  ...
  return (
    <div className="bg-slate-900/60 border-b border-slate-700/50">
      <div className="px-4 py-2 flex items-center gap-2 text-sm border-b border-slate-800/50">
        <span className="text-slate-500">Délégations</span>
