 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Inspections par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer violet par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type InspectionsModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: InspectionsModalType | null;
  data?: any;
}

interface InspectionsModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function InspectionsModals({ modal, onClose }: InspectionsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <InspectionsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <InspectionsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <InspectionsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <InspectionsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <InspectionsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <InspectionsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function InspectionsStatsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer 
      onClose={onClose} 
      title="Statistiques Inspections" 
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-violet-400" />
...
// Detail Modal
// ================================
function InspectionsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-violet-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-violet-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-violet-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-violet-500 hover:bg-violet-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function InspectionsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function InspectionsSettingsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer onClose={onClose} title="Paramètres" icon={<Settings className="w-5 h-5 text-slate-400" />}>
      <div className="space-y-6">
        ...
        <Button onClick={onClose} className="w-full bg-violet-500 hover:bg-violet-600">
          Enregistrer
        </Button>
      </div>
    </ModalContainer>
...
// Shortcuts Modal
// ================================
function InspectionsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function InspectionsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
