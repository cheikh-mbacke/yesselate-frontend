 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Assurances par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer indigo par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type AssurancesModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: AssurancesModalType | null;
  data?: any;
}

interface AssurancesModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function AssurancesModals({ modal, onClose }: AssurancesModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <AssurancesStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <AssurancesDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <AssurancesExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <AssurancesSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <AssurancesShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <AssurancesConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function AssurancesStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-indigo-400" />
...
// Detail Modal
// ================================
function AssurancesDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-indigo-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-indigo-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-indigo-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-indigo-500 hover:bg-indigo-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function AssurancesExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function AssurancesSettingsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer onClose={onClose} title="Paramètres" icon={<Settings className="w-5 h-5 text-slate-400" />}>
      <div className="space-y-6">
        ...
        <Button onClick={onClose} className="w-full bg-indigo-500 hover:bg-indigo-600">
          Enregistrer
        </Button>
      </div>
    </ModalContainer>
...
// Shortcuts Modal
// ================================
function AssurancesShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function AssurancesConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
