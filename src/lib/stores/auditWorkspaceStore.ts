/**
 * STORE: Audit Workspace - Pattern Pilotage
 */
import { create } from 'zustand';
import { persist } from 'zustand/middleware';

export type AuditTabType = 'inbox' | 'detail' | 'security' | 'financial' | 'compliance' | 'analytics';

export interface AuditTab { id: string; type: AuditTabType; title: string; icon: string; data: Record<string, unknown>; closable?: boolean; }
export interface AuditFilter { status?: string; type?: string; severity?: string; module?: string; search?: string; dateFrom?: string; dateTo?: string; }

interface AuditWorkspaceState {
  tabs: AuditTab[]; activeTabId: string | null; selectedIds: Set<string>; currentFilter: AuditFilter;
  ...
  openTab: (tab: Omit<AuditTab, 'id'> & { id?: string }) => void; closeTab: (tabId: string) => void; setActiveTab: (tabId: string) => void;
  toggleSelected: (id: string) => void; selectAll: (ids: string[]) => void; clearSelection: () => void;
  setFilter: (filter: Partial<AuditFilter>) => void; clearFilter: () => void;
...
const defaultTabs: AuditTab[] = [{ id: 'inbox:all', type: 'inbox', title: 'Tous les événements', icon: '🔍', data: { queue: 'all' }, closable: false }];

export const useAuditWorkspaceStore = create<AuditWorkspaceState>()(
  persist((set, get) => ({
  ...
  }), { name: 'audit:workspace', partialize: (s) => ({ watchlist: s.watchlist, viewMode: s.viewMode }) })
);
