...
import { Search, ArrowRight, Users, CheckCircle, AlertTriangle, UserPlus, Crown, Building2, BarChart3, RefreshCw, Download, Filter, History } from 'lucide-react';
...
export function ClientsCommandPalette({ open, onClose, onOpenStats, onRefresh }: Props) {
  const { openTab, clearFilter, setViewMode } = useClientsWorkspaceStore();
  const [query, setQuery] = useState('');
  const [selectedIndex, setSelectedIndex] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);

  const commands: Command[] = useMemo(() => [
    { id: 'nav-all', label: 'Tous les clients', icon: Users, category: 'navigation', shortcut: '⌘1', action: () => { openTab({ type: 'inbox', title: 'Tous', icon: '👥', data: { queue: 'all' } }); onClose(); } },
    { id: 'nav-active', label: 'Clients actifs', icon: CheckCircle, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:active', title: 'Actifs', icon: '✅', data: { queue: 'active' } }); onClose(); } },
    { id: 'nav-litige', label: 'En litige', description: 'Clients avec problèmes', icon: AlertTriangle, category: 'navigation', shortcut: '⌘2', action: () => { openTab({ type: 'litiges', title: 'En litige', icon: '⚠️', data: {} }); onClose(); } },
    { id: 'nav-prospect', label: 'Prospects', icon: UserPlus, category: 'navigation', action: () => { openTab({ type: 'prospects', title: 'Prospects', icon: '🆕', data: {} }); onClose(); } },
    { id: 'nav-premium', label: 'Clients Premium', icon: Crown, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:premium', title: 'Premium', icon: '👑', data: { queue: 'premium' } }); onClose(); } },
    { id: 'nav-historique', label: 'Historique relations', icon: History, category: 'navigation', action: () => { openTab({ type: 'historique', title: 'Historique', icon: '📜', data: {} }); onClose(); } },
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[12vh] bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl mx-4 rounded-2xl border border-slate-200/70 bg-white/95 backdrop-blur-xl shadow-2xl dark:border-slate-800 dark:bg-[#1f1f1f]/95 overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-200/70 dark:border-slate-800">
          <Search className="w-5 h-5 text-cyan-500 flex-none" />
