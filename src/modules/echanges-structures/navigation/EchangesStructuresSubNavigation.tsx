/**
 * Navigation secondaire et tertiaire pour le module Echanges Structures
 */
...
import {
  echangesStructuresNavigationConfig,
  getSubCategories,
  getSubSubCategories,
  type NavNode,
} from './echangesStructuresNavigationConfig';
import type { EchangesStructuresMainCategory } from '../types/echangesStructuresNavigationTypes';

interface EchangesStructuresSubNavigationProps {
...
export function EchangesStructuresSubNavigation({
  mainCategory,
  subCategory,
  subSubCategory,
  onSubCategoryChange,
  ...
  stats = {},
}: EchangesStructuresSubNavigationProps) {
  const subCategories = getSubCategories(mainCategory);
  const subSubCategories = subCategory ? getSubSubCategories(mainCategory, subCategory) : [];
  const mainLabel = echangesStructuresNavigationConfig[mainCategory]?.label || 'Échanges';
  ...
  return (
    <div className="bg-slate-900/60 border-b border-slate-700/50">
      <div className="px-4 py-2 flex items-center gap-2 text-sm border-b border-slate-800/50">
        <span className="text-slate-500">Échanges</span>
