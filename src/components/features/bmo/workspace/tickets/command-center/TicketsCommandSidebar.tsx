/**
 * Sidebar de navigation principale pour Tickets Clients
 * Inspiré de la page Analytics/Gouvernance - Centre de Commandement
 */
...
export const ticketsCategories: SidebarCategory[] = [
  ...
  { id: 'pending', label: 'En attente', icon: Clock, badge: 18, badgeType: 'warning' },
  { id: 'resolved', label: 'Résolus', icon: CheckCircle2, badge: 156 },
  { id: 'conversations', label: 'Conversations', icon: MessageSquare, badge: 8 },
...
export const TicketsCommandSidebar = React.memo(function TicketsCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Tickets</span>
          </div>
        )}
        {collapsed && (
          <Ticket className="h-5 w-5 text-purple-400 mx-auto" />
        )}
        <Button
          variant="ghost"
          size="sm"
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {ticketsCategories.map((category) => {
