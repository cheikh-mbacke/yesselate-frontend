...
import { Search, ArrowRight, ClipboardList, Clock, CheckCircle, Calendar, Users, BarChart3, RefreshCw, Download, Filter, Zap, History } from 'lucide-react';
...
export function SubstitutionCommandPalette({ open, onClose, onOpenStats, onRefresh }: Props) {
  const { openTab, clearFilter, setViewMode } = useSubstitutionWorkspaceStore();
  const [query, setQuery] = useState('');
  const [selectedIndex, setSelectedIndex] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);

  const commands: Command[] = useMemo(() => [
    { id: 'nav-all', label: 'Toutes les substitutions', icon: ClipboardList, category: 'navigation', shortcut: '⌘1', action: () => { openTab({ type: 'inbox', title: 'Toutes', icon: '🔄', data: { queue: 'all' } }); onClose(); } },
    { id: 'nav-critical', label: 'Critiques', description: 'Substitutions urgentes', icon: Zap, category: 'navigation', shortcut: '⌘2', action: () => { openTab({ type: 'inbox', id: 'inbox:critical', title: 'Critiques', icon: '⚡', data: { queue: 'critical' } }); onClose(); } },
    { id: 'nav-active', label: 'Actives', icon: RefreshCw, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:active', title: 'Actives', icon: '🔵', data: { queue: 'active' } }); onClose(); } },
    ...
    { id: 'nav-completed', label: 'Terminées', icon: CheckCircle, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:completed', title: 'Terminées', icon: '✅', data: { queue: 'completed' } }); onClose(); } },
    { id: 'nav-absences', label: 'Absences planifiées', icon: Calendar, category: 'navigation', action: () => { openTab({ type: 'absences', title: 'Absences', icon: '📅', data: {} }); onClose(); } },
    { id: 'nav-delegations', label: 'Délégations actives', icon: Users, category: 'navigation', action: () => { openTab({ type: 'delegations', title: 'Délégations', icon: '👥', data: {} }); onClose(); } },
    { id: 'nav-historique', label: 'Historique', icon: History, category: 'navigation', action: () => { openTab({ type: 'historique', title: 'Historique', icon: '📜', data: {} }); onClose(); } },
    ...
    { id: 'action-dashboard', label: 'Mode Dashboard', icon: BarChart3, category: 'action', action: () => { setViewMode('dashboard'); onClose(); } },
    { id: 'action-workspace', label: 'Mode Workspace', icon: ClipboardList, category: 'action', action: () => { setViewMode('workspace'); onClose(); } },
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[12vh] bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl mx-4 rounded-2xl border border-slate-200/70 bg-white/95 backdrop-blur-xl shadow-2xl dark:border-slate-800 dark:bg-[#1f1f1f]/95 overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-200/70 dark:border-slate-800">
          <Search className="w-5 h-5 text-indigo-500 flex-none" />
          ...
          {groupedCommands.map(({ category, items }) => (
            <div key={category} className="mb-1">
              ...
              {items.map(cmd => {
                ...
                const Icon = cmd.icon;
                return (
                  <button key={cmd.id} onClick={cmd.action} onMouseEnter={() => setSelectedIndex(globalIdx)} className={cn("w-full flex items-center gap-3 px-4 py-3 text-left", isSelected ? "bg-indigo-500/10 text-indigo-600 dark:text-indigo-400" : "hover:bg-slate-100 dark:hover:bg-slate-800/60")}>
                    <div className={cn("p-2 rounded-lg", isSelected ? "bg-indigo-500/20" : "bg-slate-100 dark:bg-slate-800")}><Icon className="w-4 h-4" /></div>
                    ...
                    {cmd.shortcut && <kbd className={cn("px-2 py-1 rounded text-xs font-mono", isSelected ? "bg-indigo-500/20" : "bg-slate-100 dark:bg-slate-800")}>{cmd.shortcut}</kbd>}
