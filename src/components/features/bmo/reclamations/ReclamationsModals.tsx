 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Reclamations par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer orange par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type ReclamationsModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: ReclamationsModalType | null;
  data?: any;
}

interface ReclamationsModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function ReclamationsModals({ modal, onClose }: ReclamationsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <ReclamationsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <ReclamationsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <ReclamationsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <ReclamationsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <ReclamationsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <ReclamationsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function ReclamationsStatsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer 
      onClose={onClose} 
      title="Statistiques Reclamations" 
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-orange-400" />
...
// Detail Modal
// ================================
function ReclamationsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-orange-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-orange-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-orange-400" />} size="large">
...
// Export Modal
// ================================
function ReclamationsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function ReclamationsSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function ReclamationsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function ReclamationsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
