 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer {{MODULE_NAME}} par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer {{MODULE_COLOR}} par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type {{MODULE_NAME}}ModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: {{MODULE_NAME}}ModalType | null;
  data?: any;
}

interface {{MODULE_NAME}}ModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function {{MODULE_NAME}}Modals({ modal, onClose }: {{MODULE_NAME}}ModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <{{MODULE_NAME}}StatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <{{MODULE_NAME}}DetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <{{MODULE_NAME}}ExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <{{MODULE_NAME}}SettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <{{MODULE_NAME}}ShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <{{MODULE_NAME}}ConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function {{MODULE_NAME}}StatsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer 
      onClose={onClose} 
      title="Statistiques {{MODULE_NAME}}" 
      icon={<BarChart3 className="w-5 h-5 text-{{MODULE_COLOR}}-400" />} 
      size="large"
    >
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-{{MODULE_COLOR}}-400" />
        </div>
      ) : stats ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-{{MODULE_COLOR}}-400" />
...
// Detail Modal
// ================================
function {{MODULE_NAME}}DetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-{{MODULE_COLOR}}-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-{{MODULE_COLOR}}-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-{{MODULE_COLOR}}-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-{{MODULE_COLOR}}-500 hover:bg-{{MODULE_COLOR}}-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function {{MODULE_NAME}}ExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function {{MODULE_NAME}}SettingsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer onClose={onClose} title="Paramètres" icon={<Settings className="w-5 h-5 text-slate-400" />}>
      <div className="space-y-6">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              className="w-4 h-4 rounded border-slate-600 text-{{MODULE_COLOR}}-500 focus:ring-{{MODULE_COLOR}}-500"
            />
          </div>
          {autoRefresh && (
            <div>
        ...
        <Button onClick={onClose} className="w-full bg-{{MODULE_COLOR}}-500 hover:bg-{{MODULE_COLOR}}-600">
          Enregistrer
        </Button>
      </div>
    </ModalContainer>
...
// Shortcuts Modal
// ================================
function {{MODULE_NAME}}ShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function {{MODULE_NAME}}ConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
