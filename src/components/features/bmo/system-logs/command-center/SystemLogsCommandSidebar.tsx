/**
 * Sidebar de navigation principale pour System Logs
 * Inspiré de AnalyticsCommandSidebar - Architecture cohérente
 */
...
const systemLogsCategories: SidebarCategory[] = [
  ...
  { id: 'security', label: 'Sécurité', icon: Shield, badge: 8, badgeType: 'warning' },
  { id: 'incidents', label: 'Incidents', icon: AlertTriangle, badge: 3, badgeType: 'critical' },
  { id: 'correlation', label: 'Corrélation', icon: SearchX },
  { id: 'integrity', label: 'Intégrité', icon: FileCheck },
  { id: 'exports', label: 'Exports', icon: Download },
  { id: 'advanced-search', label: 'Recherche avancée', icon: Search },
];
...
export const SystemLogsCommandSidebar = React.memo(function SystemLogsCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: SystemLogsCommandSidebarProps) {
  return (
    <aside
      className={cn(
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">System Logs</span>
          </div>
        )}
        {collapsed && (
          <Terminal className="h-5 w-5 text-blue-400 mx-auto" />
        )}
        <Button
          variant="ghost"
          size="sm"
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {systemLogsCategories.map((category) => {
