/**
 * Sidebar de navigation pour le module Validation-Paiements
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, DollarSign } from 'lucide-react';
import { paiementsNavigationConfig, findNavNodeById, type NavNode } from './paiementsNavigationConfig';

interface PaiementsSidebarProps {
...
export function PaiementsSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
  ...
  const getBadgeForNode = useCallback((node: NavNode): number | string | undefined => {
    ...
    const badgeMap: Record<string, number> = {
      'all': stats.pending || 0,
      'bf-pending': Math.floor((stats.pending || 0) * 0.6),
      'dg-pending': Math.floor((stats.pending || 0) * 0.4),
      'critical': Math.floor((stats.urgent || 0) * 0.4),
      'high': Math.floor((stats.urgent || 0) * 0.6),
    };
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'urgent' || node.id === 'critical' || node.id === 'dg-pending') return 'critical';
      if (node.id === 'pending' || node.id === 'high' || node.id === 'bf-pending') return 'warning';
    }
  ...
  const NavNodeComponent = React.memo(function NavNodeComponent({
    ...
    return (
      <div className={cn('w-full', level > 0 && 'ml-4')}>
        ...
        >
          {isActive && level === 0 && (
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-6 rounded-r-full bg-emerald-400" />
          )}
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(paiementsNavigationConfig).map((node) => {
            const Icon = node.icon;
      ...
      <div className="p-4 border-b border-slate-800">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Validation Paiements</h2>
          </div>
          <Button
            variant="ghost"
            size="sm"
