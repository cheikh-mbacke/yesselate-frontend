/**
 * Sidebar de navigation principale pour Substitution
...
const substitutionCategories: SidebarCategory[] = [
  ...
  { id: 'absences', label: 'Absences', icon: Calendar, badge: 8 },
  { id: 'delegations', label: 'Délégations', icon: Users, badge: 15 },
...
export const SubstitutionCommandSidebar = React.memo(function SubstitutionCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: SubstitutionCommandSidebarProps) {
  return (
    <aside
      className={cn(
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Substitution</span>
          </div>
        )}
        {collapsed && (
          <RefreshCw className="h-5 w-5 text-indigo-400 mx-auto" />
        )}
        <Button
          variant="ghost"
          size="sm"
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isActive ? 'text-indigo-400 scale-110' : 'text-slate-400 group-hover:text-slate-200 group-hover:scale-105'
                  )}
                >
