 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Exports par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer green par la couleur du module (ex: purple, emerald, blue)
...
interface ModalState {
  isOpen: boolean;
  type: ExportsModalType | null;
  data?: any;
}

interface ExportsModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function ExportsModals({ modal, onClose }: ExportsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <ExportsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <ExportsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <ExportsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <ExportsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <ExportsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <ExportsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function ExportsStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-green-400" />
...
// Detail Modal
// ================================
function ExportsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-green-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-green-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-green-400" />} size="large">
...
// Export Modal
// ================================
function ExportsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function ExportsSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function ExportsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function ExportsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
