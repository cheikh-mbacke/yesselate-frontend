/**
 * Sidebar de navigation principale pour Logs
...
const logsCategories: SidebarCategory[] = [
  { id: 'overview', label: "Vue d'ensemble", icon: LayoutDashboard },
  { id: 'errors', label: 'Erreurs', icon: AlertCircle, badge: 24, badgeType: 'critical' },
  { id: 'warnings', label: 'Avertissements', icon: AlertTriangle, badge: 12, badgeType: 'warning' },
  { id: 'system', label: 'Système', icon: Server },
  { id: 'api', label: 'API', icon: Globe },
  { id: 'security', label: 'Sécurité', icon: Shield, badge: 3, badgeType: 'critical' },
  { id: 'audit', label: 'Audit', icon: FileText },
  { id: 'user-actions', label: 'Actions utilisateur', icon: User },
  { id: 'analysis', label: 'Analyse', icon: BarChart3 },
];

interface LogsCommandSidebarProps {
...
export const LogsCommandSidebar = React.memo(function LogsCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: LogsCommandSidebarProps) {
  return (
    <aside
      className={cn(
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Logs</span>
          </div>
        )}
        {collapsed && (
