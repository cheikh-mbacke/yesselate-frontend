/**
 * Assurances Notification Panel
 * Panneau de notifications pour Assurances
 */
...
interface AssurancesNotificationPanelProps {
  isOpen: boolean;
  onClose: () => void;
}

export function AssurancesNotificationPanel({ isOpen, onClose }: AssurancesNotificationPanelProps) {
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      id: '1',
      type: 'urgent',
      title: 'Notification urgente',
      message: 'Exemple de notification urgente pour Assurances',
      ...
      <div className="fixed right-0 top-0 bottom-0 w-96 bg-slate-900 border-l border-slate-700/50 shadow-2xl z-50 flex flex-col animate-slideInRight">
        ...
        <div className="px-4 py-3 border-t border-slate-800 bg-slate-900/80">
          <div className="flex items-center justify-between text-xs text-slate-500">
            ...
            {unreadCount > 0 && (
              <span className="text-indigo-400">{unreadCount} non lu{unreadCount > 1 ? 'es' : 'e'}</span>
            )}
          </div>
        </div>
      </div>
