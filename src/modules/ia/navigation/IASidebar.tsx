/**
 * Sidebar de navigation pour le module IA
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, Brain } from 'lucide-react';
import { iaNavigationConfig, type NavNode } from './iaNavigationConfig';
...
export function IASidebar({
  ...
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set(['modules']));
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' | 'success' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'error' || node.id === 'anomaly') return 'critical';
      if (node.id === 'training' || node.id === 'recommendations') return 'warning';
      if (node.id === 'active') return 'success';
    }
    ...
    const handleClick = () => {
      if (hasChildren) {
        ...
        if (level === 0 && node.children && node.children.length > 0) {
          onCategoryChange(node.id as IAMainCategory, node.children[0].id);
        }
      } else {
        if (level === 1) onCategoryChange(activeCategory, node.id);
        else onCategoryChange(node.id as IAMainCategory);
      }
    };
        ...
        <button onClick={handleClick} className={cn('w-full flex items-center gap-2 px-3 py-2 rounded-lg transition-all duration-200 text-left', 'group relative', isActive ? 'bg-blue-500/10 border border-blue-500/30 text-blue-300' : 'hover:bg-slate-700/40 border border-transparent text-slate-300')}>
          ...
          {!collapsed && (
            <>
              ...
              {badge !== undefined && badge !== null && badge !== 0 && <Badge variant={badgeType === 'critical' ? 'urgent' : badgeType === 'warning' ? 'warning' : badgeType === 'success' ? 'success' : 'default'} className="h-5 min-w-5 px-1.5 text-xs font-medium">{badge}</Badge>}
  ...
  return (
    <aside className={cn('flex flex-col border-r border-slate-700/50 bg-slate-900/80 backdrop-blur-xl transition-all duration-300', 'fixed sm:relative z-40 h-full', collapsed ? 'w-16' : 'w-64')}>
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && <div className="flex items-center gap-2"><Brain className="h-5 w-5 text-purple-400" /><span className="font-semibold text-slate-200 text-sm">IA</span></div>}
        {collapsed && <Brain className="h-5 w-5 text-purple-400 mx-auto" />}
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {Object.values(iaNavigationConfig).map((node) => <NavNodeComponent key={node.id} node={node} level={0} />)}
        </div>
      </nav>
      {!collapsed && <div className="border-t border-slate-700/50 p-3"><div className="text-xs text-slate-500 text-center">IA v2.0</div></div>}
    </aside>
  );
}
