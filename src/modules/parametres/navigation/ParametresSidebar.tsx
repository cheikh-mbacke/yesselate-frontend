/**
 * Sidebar de navigation pour le module Paramètres
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, Settings } from 'lucide-react';
import { parametresNavigationConfig, type NavNode } from './parametresNavigationConfig';
import type { ParametresMainCategory } from '../types/parametresNavigationTypes';

interface ParametresSidebarProps {
  activeCategory: ParametresMainCategory;
...
}: ParametresSidebarProps) {
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set(['general']));
    ...
    const handleClick = () => {
      if (hasChildren) {
        ...
        if (level === 0 && node.children && node.children.length > 0) {
          onCategoryChange(node.id as ParametresMainCategory, node.children[0].id);
        }
      } else {
        if (level === 1) onCategoryChange(activeCategory, node.id);
        else onCategoryChange(node.id as ParametresMainCategory);
      }
    };
  ...
  return (
    <aside className={cn('flex flex-col border-r border-slate-700/50 bg-slate-900/80 backdrop-blur-xl transition-all duration-300', 'fixed sm:relative z-40 h-full', collapsed ? 'w-16' : 'w-64')}>
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && <div className="flex items-center gap-2"><Settings className="h-5 w-5 text-slate-400" /><span className="font-semibold text-slate-200 text-sm">Paramètres</span></div>}
        {collapsed && <Settings className="h-5 w-5 text-slate-400 mx-auto" />}
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {Object.values(parametresNavigationConfig).map((node) => <NavNodeComponent key={node.id} node={node} level={0} />)}
        </div>
      </nav>
      {!collapsed && <div className="border-t border-slate-700/50 p-3"><div className="text-xs text-slate-500 text-center">Paramètres v2.0</div></div>}
    </aside>
  );
}
