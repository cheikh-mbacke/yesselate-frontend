 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Fournisseurs par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer teal par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type FournisseursModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: FournisseursModalType | null;
  data?: any;
}
...
// Main Modal Router
// ================================
export function FournisseursModals({ modal, onClose }: FournisseursModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <FournisseursStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <FournisseursDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <FournisseursExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <FournisseursSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <FournisseursShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <FournisseursConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function FournisseursStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-teal-400" />
...
// Detail Modal
// ================================
function FournisseursDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-teal-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-teal-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-teal-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-teal-500 hover:bg-teal-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function FournisseursExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function FournisseursSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function FournisseursShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function FournisseursConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
