/**
 * Sidebar de navigation principale pour Conférences
...
const conferencesCategories: SidebarCategory[] = [
  { id: 'overview', label: "Vue d'ensemble", icon: LayoutDashboard },
  { id: 'planned', label: 'Planifiées', icon: Calendar, badge: 0 },
  { id: 'ongoing', label: 'En cours', icon: Clock, badge: 0 },
  { id: 'completed', label: 'Terminées', icon: CheckCircle2 },
  { id: 'crisis', label: 'Crise', icon: AlertTriangle, badge: 0, badgeType: 'critical' },
  { id: 'arbitrage', label: 'Arbitrage', icon: Gavel, badge: 0 },
  { id: 'revue_projet', label: 'Revue projet', icon: Briefcase, badge: 0 },
  { id: 'comite_direction', label: 'Comité direction', icon: Users, badge: 0 },
  { id: 'resolution_blocage', label: 'Résolution blocage', icon: Unlock, badge: 0 },
];
...
interface ConferencesCommandSidebarProps {
  ...
  stats?: {
    ...
    arbitrage: number;
    revue_projet: number;
    comite_direction: number;
    resolution_blocage: number;
  };
}

export const ConferencesCommandSidebar = React.memo(function ConferencesCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  ...
  stats,
}: ConferencesCommandSidebarProps) {
  // Mapper les stats aux badges
  const categoriesWithStats = conferencesCategories.map((cat) => {
    if (!stats) return cat;
    
    switch (cat.id) {
      case 'planned':
        return { ...cat, badge: stats.planifiees };
      case 'ongoing':
        return { ...cat, badge: stats.enCours };
      case 'crisis':
        return { ...cat, badge: stats.crise, badgeType: stats.crise > 0 ? 'critical' : 'default' };
      case 'arbitrage':
        return { ...cat, badge: stats.arbitrage };
      case 'revue_projet':
        return { ...cat, badge: stats.revue_projet };
      case 'comite_direction':
        return { ...cat, badge: stats.comite_direction };
      case 'resolution_blocage':
        return { ...cat, badge: stats.resolution_blocage };
      default:
        return cat;
    }
  });
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Conférences</span>
          </div>
        )}
        {collapsed && (
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isActive ? 'text-purple-400 scale-110' : 'text-slate-400 group-hover:text-slate-200 group-hover:scale-105'
                  )}
                >
