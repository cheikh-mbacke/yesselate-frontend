...
export function RecouvrementsCommandPalette({ open, onClose, onOpenStats, onRefresh }: Props) {
  ...
  const commands: Command[] = useMemo(() => [
    { id: 'nav-all', label: 'Toutes les créances', icon: ClipboardList, category: 'navigation', shortcut: '⌘1', action: () => { openTab({ type: 'inbox', title: 'Toutes', icon: '💰', data: { queue: 'all' } }); onClose(); } },
    ...
    { id: 'nav-overdue', label: 'En retard', icon: CalendarClock, category: 'navigation', shortcut: '⌘2', action: () => { openTab({ type: 'inbox', id: 'inbox:overdue', title: 'En retard', icon: '⏰', data: { queue: 'overdue' } }); onClose(); } },
    { id: 'nav-litige', label: 'En litige', icon: AlertTriangle, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:litige', title: 'Litige', icon: '⚠️', data: { queue: 'litige' } }); onClose(); } },
    { id: 'nav-paid', label: 'Payées', icon: CheckCircle, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:paid', title: 'Payées', icon: '✅', data: { queue: 'paid' } }); onClose(); } },
    { id: 'nav-relances', label: 'Suivi relances', icon: Bell, category: 'navigation', action: () => { openTab({ type: 'relances', title: 'Relances', icon: '🔔', data: {} }); onClose(); } },
