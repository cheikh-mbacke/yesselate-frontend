 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Projets par le nom du module (ex: Delegations, Finances)
...
// Main Modal Router
// ================================
export function ProjetsModals({ modal, onClose }: ProjetsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <ProjetsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <ProjetsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <ProjetsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <ProjetsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <ProjetsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <ProjetsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function ProjetsStatsModal({ onClose }: { onClose: () => void }) {
...
// Detail Modal
// ================================
function ProjetsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
...
// Export Modal
// ================================
function ProjetsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function ProjetsSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function ProjetsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function ProjetsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
