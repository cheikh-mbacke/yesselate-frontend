/**
 * Navigation secondaire et tertiaire pour le module Tickets-Clients
 */
...
export function TicketsSubNavigation({
  ...
  const mainLabel = ticketsNavigationConfig[mainCategory]?.label || 'Tickets';
  ...
  const getBadgeTypeForNode = (node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'critical' || node.id === 'unassigned' || node.id === 'breached') return 'critical';
      if (node.id === 'open' || node.id === 'pending' || node.id === 'high') return 'warning';
    }
  ...
  return (
    <div className="bg-slate-900/60 border-b border-slate-700/50">
      <div className="px-4 py-2 flex items-center gap-2 text-sm border-b border-slate-800/50">
        <span className="text-slate-500">Tickets Clients</span>
