...
const tabIcons: Record<string, React.ReactNode> = {
  ...
  mission: <FileText className="w-4 h-4" />,
  calendrier: <Calendar className="w-4 h-4" />,
  frais: <Receipt className="w-4 h-4" />,
  validation: <CheckSquare className="w-4 h-4" />,
};

export function MissionsWorkspaceTabs() {
  const { tabs, activeTabId, setActiveTab, closeTab } = useMissionsWorkspaceStore();
