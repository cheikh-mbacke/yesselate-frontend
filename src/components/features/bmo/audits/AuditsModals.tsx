 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Audits par le nom du module (ex: Delegations, Finances)
...
interface ModalState {
  isOpen: boolean;
  type: AuditsModalType | null;
  data?: any;
}

interface AuditsModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function AuditsModals({ modal, onClose }: AuditsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <AuditsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <AuditsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <AuditsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <AuditsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <AuditsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <AuditsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function AuditsStatsModal({ onClose }: { onClose: () => void }) {
...
// Detail Modal
// ================================
function AuditsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
...
// Export Modal
// ================================
function AuditsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function AuditsSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function AuditsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function AuditsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
