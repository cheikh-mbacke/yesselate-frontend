 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Archives par le nom du module (ex: Delegations, Finances)
...
interface ModalState {
  isOpen: boolean;
  type: ArchivesModalType | null;
  data?: any;
}

interface ArchivesModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function ArchivesModals({ modal, onClose }: ArchivesModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <ArchivesStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <ArchivesDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <ArchivesExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <ArchivesSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <ArchivesShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <ArchivesConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function ArchivesStatsModal({ onClose }: { onClose: () => void }) {
...
// Detail Modal
// ================================
function ArchivesDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
...
// Export Modal
// ================================
function ArchivesExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function ArchivesSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function ArchivesShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function ArchivesConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
