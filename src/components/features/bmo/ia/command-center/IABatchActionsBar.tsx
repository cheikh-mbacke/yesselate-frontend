/**
 * Barre d'actions groupées pour IA
 * Apparaît quand des modules sont sélectionnés
 */
...
const defaultActions = [
  ...
  { id: 'activate', icon: Power, label: 'Activer', variant: 'default' as const },
  { id: 'deactivate', icon: PowerOff, label: 'Désactiver', variant: 'warning' as const },
  { id: 'retrain', icon: RefreshCw, label: 'Ré-entraîner', variant: 'default' as const },
...
export function IABatchActionsBar({
  onAction,
  actions = defaultActions,
}: BatchActionsBarProps) {
  const { selectedItems, clearSelection } = useIACommandCenterStore();
