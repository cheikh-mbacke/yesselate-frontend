/**
 * Sidebar de navigation principale pour Arbitrages & Goulots
 * Architecture Command Center - Version 3.0
 */
...
export const arbitragesCategories: SidebarCategory[] = [
  ...
  { id: 'escalated', label: 'Escaladés', icon: AlertTriangle, badge: 7, badgeType: 'warning' },
  { id: 'goulots', label: 'Goulots', icon: Target },
  { id: 'categories', label: 'Par catégorie', icon: Workflow },
...
export const ArbitragesCommandSidebar = React.memo(function ArbitragesCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: ArbitragesCommandSidebarProps) {
  return (
    <aside
      className={cn(
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Arbitrages</span>
          </div>
        )}
        {collapsed && (
          <Scale className="h-5 w-5 text-orange-400 mx-auto" />
        )}
        <Button
          variant="ghost"
          size="sm"
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {arbitragesCategories.map((category) => {
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  'group relative',
                  isActive
                    ? 'bg-orange-500/10 border border-orange-500/30 scale-[1.02]'
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isActive ? 'text-orange-400 scale-110' : 'text-slate-400 group-hover:text-slate-200 group-hover:scale-105'
                  )}
                >
