/**
 * Sidebar de navigation pour le module Centre d'Alerte
...
import { alertesNavigation, findNavNodeById, type AlerteNavItem } from './alertesNavigationConfig';
import { useAlertesStats } from '../hooks/useAlertesStats';
...
export function AlertesSidebar({
  collapsed = false,
  onToggleCollapse,
}: AlertesSidebarProps) {
  ...
  const getBadgeForNode = useCallback((node: AlerteNavItem): number | string | undefined => {
  ...
  const getBadgeTypeForNode = useCallback((node: AlerteNavItem): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      ...
      if (node.id === 'sla' || node.id === 'rh' || node.id === 'projets') return 'warning';
    }
  ...
  const isNodeActive = useCallback((node: AlerteNavItem): boolean => {
  ...
  useEffect(() => {
    ...
    const checkAndExpand = (nodes: AlerteNavItem[], parentId?: string) => {
      nodes.forEach((node) => {
    ...
    checkAndExpand(alertesNavigation);
    ...
    >
      ...
      {!collapsed && (
        <div className="px-4 py-3 border-b border-slate-700/50">
          <h2 className="text-sm font-semibold text-slate-200">Centre d'Alertes</h2>
        </div>
      )}

      {/* Navigation */}
      <nav className="flex-1 overflow-y-auto p-2 space-y-1">
        {alertesNavigation.map((node) => (
