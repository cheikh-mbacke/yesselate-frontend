/**
 * Sidebar de navigation pour le module DemandesRH
 */
...
import { ChevronDown, ChevronRight, ChevronLeft, Search, FileText } from 'lucide-react';
import { demandesRhNavigationConfig, type NavNode } from './demandesRhNavigationConfig';

interface DemandesRhSidebarProps {
...
export function DemandesRhSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'urgent' || node.id === 'pending' || node.id === 'critical') return 'critical';
      if (node.id === 'in-progress' || node.id === 'active') return 'warning';
    }
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(demandesRhNavigationConfig).map((node) => {
            const Icon = node.icon;
      ...
      <div className="p-4 border-b border-slate-800">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Demandes RH</h2>
          </div>
          <Button
            variant="ghost"
            size="sm"
