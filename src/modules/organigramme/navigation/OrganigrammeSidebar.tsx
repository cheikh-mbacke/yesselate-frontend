/**
 * Sidebar de navigation pour le module Organigramme
 */
...
import { organigrammeNavigationConfig, type NavNode } from './organigrammeNavigationConfig';

interface OrganigrammeSidebarProps {
...
export function OrganigrammeSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'executive' || node.id === 'management') return 'critical';
      if (node.id === 'total' || node.id === 'departments') return 'warning';
    }
    ...
    return (
      <div className={cn('w-full', level > 0 && 'ml-4')}>
        ...
        >
          ...
          {Icon && (
            <Icon className={cn('h-4 w-4 flex-shrink-0', isActive ? 'text-sky-400' : 'text-slate-400')} />
          )}
          {!collapsed && (
            <>
    ...
    return (
      <div className="w-16 h-full bg-slate-900/80 border-r border-slate-800 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {Object.values(organigrammeNavigationConfig).map((node) => {
            const Icon = node.icon;
      ...
      <div className="p-4 border-b border-slate-800">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold text-slate-200">Organigramme</h2>
          </div>
          <Button
            variant="ghost"
            size="sm"
