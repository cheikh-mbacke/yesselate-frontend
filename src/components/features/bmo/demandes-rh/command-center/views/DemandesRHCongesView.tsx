/**
 * Vue Congés - Liste des demandes de congés
 */
...
interface DemandesRHCongesViewProps {
  subCategory?: string;
}

export function DemandesRHCongesView({ subCategory = 'all' }: DemandesRHCongesViewProps) {
  ...
  useEffect(() => {
    setLoading(true);
    // Simuler appel API
    fetch('/api/rh/demandes?type=conges')
      .then((res) => res.json())
      .then((data) => {
      ...
      <div className="space-y-2">
        {filteredDemandes.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-slate-400">Aucune demande de congés</p>
          </div>
        ) : (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <h3 className="text-base font-semibold text-slate-200 mb-1 group-hover:text-blue-400 transition-colors">
                    {demande.objet}
                  </h3>
                  <div className="flex items-center gap-4 text-sm text-slate-400">
                    ...
                    {demande.dateDebut && demande.dateFin && (
                      <div className="flex items-center gap-1.5">
                        ...
                        {demande.duree && <span>({demande.duree} jour(s))</span>}
                      </div>
                    )}
                    <div className="flex items-center gap-1.5">
