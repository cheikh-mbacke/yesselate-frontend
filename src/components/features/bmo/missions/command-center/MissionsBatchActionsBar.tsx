 ...
 * Apparaît quand des missions sont sélectionnées
 */
...
const defaultActions = [
  ...
  { id: 'start', icon: PlayCircle, label: 'Démarrer', variant: 'default' as const },
  { id: 'cancel', icon: XCircle, label: 'Annuler', variant: 'warning' as const },
...
export function MissionsBatchActionsBar({
  onAction,
  actions = defaultActions,
}: BatchActionsBarProps) {
  const { selectedItems, clearSelection } = useMissionsCommandCenterStore();
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-slate-900/95 border border-slate-700/50 shadow-xl backdrop-blur-lg">
        ...
        <div className="flex items-center gap-2 pr-3 border-r border-slate-700/50">
          ...
          <span className="text-sm text-slate-300">
            mission{selectedItems.length > 1 ? 's' : ''} sélectionnée{selectedItems.length > 1 ? 's' : ''}
          </span>
        </div>
