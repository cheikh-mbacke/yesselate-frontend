 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Notifications par le nom du module (ex: Delegations, Finances)
...
interface ModalState {
  isOpen: boolean;
  type: NotificationsModalType | null;
  data?: any;
}
...
// Main Modal Router
// ================================
export function NotificationsModals({ modal, onClose }: NotificationsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <NotificationsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <NotificationsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <NotificationsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <NotificationsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <NotificationsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <NotificationsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function NotificationsStatsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer 
      onClose={onClose} 
      title="Statistiques Notifications" 
...
// Detail Modal
// ================================
function NotificationsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
...
// Export Modal
// ================================
function NotificationsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function NotificationsSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function NotificationsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function NotificationsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
