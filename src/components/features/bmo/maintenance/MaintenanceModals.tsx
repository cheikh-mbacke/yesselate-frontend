 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Maintenance par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer slate par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type MaintenanceModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: MaintenanceModalType | null;
  data?: any;
}
...
// Main Modal Router
// ================================
export function MaintenanceModals({ modal, onClose }: MaintenanceModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <MaintenanceStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <MaintenanceDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <MaintenanceExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <MaintenanceSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <MaintenanceShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <MaintenanceConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function MaintenanceStatsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer 
      onClose={onClose} 
      title="Statistiques Maintenance" 
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-slate-400" />
...
// Detail Modal
// ================================
function MaintenanceDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-slate-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-slate-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-slate-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-slate-500 hover:bg-slate-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function MaintenanceExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function MaintenanceSettingsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer onClose={onClose} title="Paramètres" icon={<Settings className="w-5 h-5 text-slate-400" />}>
      <div className="space-y-6">
        ...
        <Button onClick={onClose} className="w-full bg-slate-500 hover:bg-slate-600">
          Enregistrer
        </Button>
      </div>
    </ModalContainer>
...
// Shortcuts Modal
// ================================
function MaintenanceShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function MaintenanceConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
