 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Garanties par le nom du module (ex: Delegations, Finances)
...
// ================================
export type GarantiesModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: GarantiesModalType | null;
  data?: any;
}

interface GarantiesModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function GarantiesModals({ modal, onClose }: GarantiesModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <GarantiesStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <GarantiesDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <GarantiesExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <GarantiesSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <GarantiesShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <GarantiesConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function GarantiesStatsModal({ onClose }: { onClose: () => void }) {
...
// Detail Modal
// ================================
function GarantiesDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
...
// Export Modal
// ================================
function GarantiesExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function GarantiesSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function GarantiesShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function GarantiesConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
