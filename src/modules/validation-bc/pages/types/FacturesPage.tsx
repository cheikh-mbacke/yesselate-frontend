...
export function FacturesPage() {
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <div className="text-slate-400">Chargement des factures...</div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6">
        <div className="text-red-400">Erreur lors du chargement des factures</div>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="text-lg font-semibold text-slate-200">Factures</h2>
        </div>
        <p className="text-sm text-slate-400">
          Liste des factures en attente de validation ({documents?.length || 0})
        </p>
      </div>
      ...
      {documents && documents.length > 0 ? (
        ...
        <div className="p-8 rounded-lg border border-slate-700/50 bg-slate-800/30 text-center">
          <div className="text-slate-400">Aucune facture trouvée</div>
        </div>
      )}
