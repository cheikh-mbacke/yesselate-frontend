...
export function LitigesCommandPalette({ open, onClose, onOpenStats, onRefresh }: Props) {
  const { openTab, setFilter, clearFilter } = useLitigesWorkspaceStore();
  const [query, setQuery] = useState('');
  const [selectedIndex, setSelectedIndex] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);

  const commands: Command[] = useMemo(() => [
    { id: 'nav-all', label: 'Tous les litiges', icon: ClipboardList, category: 'navigation', shortcut: '⌘1', action: () => { openTab({ type: 'inbox', title: 'Tous', icon: '⚖️', data: { queue: 'all' } }); onClose(); } },
    { id: 'nav-active', label: 'Litiges actifs', icon: Scale, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:active', title: 'Actifs', icon: '⚖️', data: { queue: 'active' } }); onClose(); } },
    { id: 'nav-high-risk', label: 'Risque élevé', icon: AlertTriangle, category: 'navigation', shortcut: '⌘2', action: () => { openTab({ type: 'inbox', id: 'inbox:high-risk', title: 'Risque élevé', icon: '🚨', data: { queue: 'high-risk' } }); onClose(); } },
    { id: 'nav-negotiation', label: 'En négociation', icon: MessageSquare, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:negotiation', title: 'Négociation', icon: '💬', data: { queue: 'negotiation' } }); onClose(); } },
    { id: 'nav-judgment', label: 'Jugements rendus', icon: Gavel, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:judgment', title: 'Jugements', icon: '🔨', data: { queue: 'judgment' } }); onClose(); } },
    { id: 'nav-closed', label: 'Litiges clos', icon: CheckCircle, category: 'navigation', action: () => { openTab({ type: 'inbox', id: 'inbox:closed', title: 'Clos', icon: '✅', data: { queue: 'closed' } }); onClose(); } },
    { id: 'nav-calendar', label: 'Calendrier audiences', icon: Calendar, category: 'navigation', action: () => { openTab({ type: 'calendrier', title: 'Audiences', icon: '📅', data: {} }); onClose(); } },
    { id: 'nav-risks', label: 'Analyse des risques', icon: AlertTriangle, category: 'navigation', action: () => { openTab({ type: 'risques', title: 'Risques', icon: '⚠️', data: {} }); onClose(); } },
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center pt-[12vh] bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-2xl mx-4 rounded-2xl border border-slate-200/70 bg-white/95 backdrop-blur-xl shadow-2xl dark:border-slate-800 dark:bg-[#1f1f1f]/95 overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-200/70 dark:border-slate-800">
          <Search className="w-5 h-5 text-red-500 flex-none" />
          ...
          {groupedCommands.map(({ category, items }) => (
            <div key={category} className="mb-1">
              ...
              {items.map(cmd => {
                ...
                const Icon = cmd.icon;
                return (
                  <button key={cmd.id} onClick={cmd.action} onMouseEnter={() => setSelectedIndex(globalIdx)} className={cn("w-full flex items-center gap-3 px-4 py-3 text-left", isSelected ? "bg-red-500/10 text-red-600 dark:text-red-400" : "hover:bg-slate-100 dark:hover:bg-slate-800/60")}>
                    <div className={cn("p-2 rounded-lg", isSelected ? "bg-red-500/20" : "bg-slate-100 dark:bg-slate-800")}><Icon className="w-4 h-4" /></div>
                    <div className="flex-1 min-w-0"><div className="font-medium">{cmd.label}</div>{cmd.description && <div className="text-xs text-slate-500 truncate">{cmd.description}</div>}</div>
                    {cmd.shortcut && <kbd className={cn("px-2 py-1 rounded text-xs font-mono", isSelected ? "bg-red-500/20" : "bg-slate-100 dark:bg-slate-800")}>{cmd.shortcut}</kbd>}
