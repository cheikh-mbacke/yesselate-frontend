/**
 * Navigation secondaire et tertiaire pour le module Employes
 */
...
import {
  employesNavigationConfig,
  getSubCategories,
  getSubSubCategories,
  type NavNode,
} from './employesNavigationConfig';
import type { EmployesMainCategory } from '../types/employesTypes';

interface EmployesSubNavigationProps {
...
export function EmployesSubNavigation({
  mainCategory,
  subCategory,
  subSubCategory,
  onSubCategoryChange,
  ...
  stats = {},
}: EmployesSubNavigationProps) {
  const subCategories = getSubCategories(mainCategory);
  const subSubCategories = subCategory ? getSubSubCategories(mainCategory, subCategory) : [];

  const mainLabel = employesNavigationConfig[mainCategory]?.label || 'Employés';
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-teal-500/15 text-slate-200 border border-teal-500/30 scale-105'
