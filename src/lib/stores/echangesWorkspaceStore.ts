/**
 * STORE: Échanges Inter-Bureaux Workspace - Pattern Pilotage
 */
import { create } from 'zustand';
import { persist } from 'zustand/middleware';

export type EchangesTabType = 'inbox' | 'detail' | 'escaladed' | 'urgent' | 'projets' | 'analytics';

export interface EchangesTab { id: string; type: EchangesTabType; title: string; icon: string; data: Record<string, unknown>; closable?: boolean; }
export interface EchangesFilter { status?: string; priority?: string; bureauFrom?: string; bureauTo?: string; search?: string; }

interface EchangesWorkspaceState {
  tabs: EchangesTab[]; activeTabId: string | null; selectedIds: Set<string>; currentFilter: EchangesFilter;
  ...
  openTab: (tab: Omit<EchangesTab, 'id'> & { id?: string }) => void; closeTab: (tabId: string) => void; setActiveTab: (tabId: string) => void;
  toggleSelected: (id: string) => void; selectAll: (ids: string[]) => void; clearSelection: () => void;
  setFilter: (filter: Partial<EchangesFilter>) => void; clearFilter: () => void;
...
const defaultTabs: EchangesTab[] = [{ id: 'inbox:all', type: 'inbox', title: 'Tous les échanges', icon: '📨', data: { queue: 'all' }, closable: false }];

export const useEchangesWorkspaceStore = create<EchangesWorkspaceState>()(
  persist((set, get) => ({
  ...
  }), { name: 'echanges:workspace', partialize: (s) => ({ watchlist: s.watchlist, viewMode: s.viewMode }) })
);
