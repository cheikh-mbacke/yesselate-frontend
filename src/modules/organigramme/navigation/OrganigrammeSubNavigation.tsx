/**
 * Navigation secondaire et tertiaire pour le module Organigramme
 */
...
import {
  organigrammeNavigationConfig,
  getSubCategories,
  getSubSubCategories,
  type NavNode,
} from './organigrammeNavigationConfig';
import type { OrganigrammeMainCategory } from '../types/organigrammeTypes';

interface OrganigrammeSubNavigationProps {
...
export function OrganigrammeSubNavigation({
  mainCategory,
  subCategory,
  subSubCategory,
  onSubCategoryChange,
  ...
  stats = {},
}: OrganigrammeSubNavigationProps) {
  const subCategories = getSubCategories(mainCategory);
  const subSubCategories = subCategory ? getSubSubCategories(mainCategory, subCategory) : [];

  const mainLabel = organigrammeNavigationConfig[mainCategory]?.label || 'Organigramme';
  ...
  return (
    <div className="bg-slate-900/60 border-b border-slate-700/50">
      <div className="px-4 py-2 flex items-center gap-2 text-sm border-b border-slate-800/50">
        <span className="text-slate-500">Organigramme</span>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive
                    ? 'bg-sky-500/15 text-slate-200 border border-sky-500/30 scale-105'
