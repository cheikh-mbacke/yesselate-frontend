...
interface UrgentsPageProps {
  filterService?: string;
}

export function UrgentsPage({ filterService }: UrgentsPageProps = {}) {
  const { data: contrats, isLoading, error } = useContratsByStatut('URGENT');
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <div className="text-slate-400">Chargement des contrats urgents...</div>
      </div>
    );
  }
    ...
    return (
      <div className="p-6">
        <div className="flex items-center gap-2 text-slate-400">
          ...
          <span>Aucun contrat urgent</span>
        </div>
      </div>
    );
  }
