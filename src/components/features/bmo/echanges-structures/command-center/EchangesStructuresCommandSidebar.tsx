/**
 * Sidebar de navigation principale pour Échanges Structurés
...
const echangesStructuresCategories: SidebarCategory[] = [
  ...
  { id: 'escalade', label: 'Escaladés', icon: AlertTriangle, badge: stats.escalades, badgeType: 'critical' },
  { id: 'resolu', label: 'Résolus', icon: CheckCircle2 },
  { id: 'critiques', label: 'Critiques', icon: AlertCircle, badge: stats.critiques, badgeType: 'critical' },
  { id: 'en_retard', label: 'En retard', icon: Timer, badge: stats.enRetard, badgeType: 'critical' },
...
export const EchangesStructuresCommandSidebar = React.memo(function EchangesStructuresCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: EchangesStructuresCommandSidebarProps) {
  return (
    <aside
      className={cn(
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Échanges Structurés</span>
          </div>
        )}
        {collapsed && (
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {echangesStructuresCategories.map((category) => {
