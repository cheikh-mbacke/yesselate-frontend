/**
 * Barre de KPIs en temps réel pour Organigramme
...
const mockKPIs: KPIItem[] = [
  ...
  },
  {
    ...
    trendValue: '+3',
    status: 'success',
    sparkline: [40, 41, 42, 43, 44, 44, 45],
  },
  {
    id: 'equipes-total',
    label: 'Équipes',
  ...
  },
  {
    ...
    trendValue: '+3%',
    status: 'success',
    sparkline: [82, 83, 84, 85, 86, 86, 87],
  },
  {
    id: 'budget-execution',
    label: 'Budget',
...
interface OrganigrammeKPIBarProps {
  visible?: boolean;
  collapsed?: boolean;
...
export const OrganigrammeKPIBar = React.memo(function OrganigrammeKPIBar({
  visible = true,
  collapsed = false,
  onToggleCollapse,
  onRefresh,
}: OrganigrammeKPIBarProps) {
...
const KPICard = React.memo(function KPICard({ kpi }: { kpi: KPIItem }) {
  const { openDetailPanel, openModal } = useOrganigrammeCommandCenterStore();
  ...
  const handleClick = () => {
    openDetailPanel('position', kpi.id, {
      name: kpi.label,
      value: kpi.value,
      trend: kpi.trend,
