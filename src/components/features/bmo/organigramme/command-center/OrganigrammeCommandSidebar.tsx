/**
 * Sidebar de navigation principale pour Organigramme
...
const organigrammeCategories: SidebarCategory[] = [
  { id: 'overview', label: "Vue d'ensemble", icon: LayoutDashboard },
  { id: 'hierarchy', label: 'Hiérarchie', icon: Network },
  { id: 'changes', label: 'Changements', icon: GitBranch, badge: 12 },
  { id: 'positions', label: 'Postes', icon: Briefcase, badge: 45 },
  { id: 'bureaux', label: 'Bureaux', icon: Building2, badge: 8 },
  { id: 'teams', label: 'Équipes', icon: UserCheck, badge: 24 },
  { id: 'skills', label: 'Compétences', icon: Award },
  { id: 'history', label: 'Historique', icon: History },
  { id: 'analytics', label: 'Analyse', icon: BarChart3 },
];

interface OrganigrammeCommandSidebarProps {
...
export const OrganigrammeCommandSidebar = React.memo(function OrganigrammeCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: OrganigrammeCommandSidebarProps) {
  return (
    <aside
      className={cn(
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Organigramme</span>
          </div>
        )}
        {collapsed && (
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {organigrammeCategories.map((category) => {
