 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Rapports par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer blue par la couleur du module (ex: purple, emerald, blue)
...
interface ModalState {
  isOpen: boolean;
  type: RapportsModalType | null;
  data?: any;
}

interface RapportsModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function RapportsModals({ modal, onClose }: RapportsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <RapportsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <RapportsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <RapportsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <RapportsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <RapportsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <RapportsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function RapportsStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-blue-400" />
...
// Detail Modal
// ================================
function RapportsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-blue-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-blue-500 hover:bg-blue-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function RapportsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function RapportsSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function RapportsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function RapportsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
