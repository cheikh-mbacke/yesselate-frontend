/**
 * Parametres Notification Panel
 * Panneau de notifications pour Parametres
 */
...
interface ParametresNotificationPanelProps {
  isOpen: boolean;
  onClose: () => void;
}

export function ParametresNotificationPanel({ isOpen, onClose }: ParametresNotificationPanelProps) {
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      id: '1',
      type: 'urgent',
      title: 'Notification urgente',
      message: 'Exemple de notification urgente pour Parametres',
      ...
      <div className="fixed right-0 top-0 bottom-0 w-96 bg-slate-900 border-l border-slate-700/50 shadow-2xl z-50 flex flex-col animate-slideInRight">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      {!notif.read && <div className="w-2 h-2 rounded-full bg-cyan-400 flex-shrink-0 mt-1" />}
                    </div>
        ...
        <div className="px-4 py-3 border-t border-slate-800 bg-slate-900/80">
          <div className="flex items-center justify-between text-xs text-slate-500">
            ...
            {unreadCount > 0 && (
              <span className="text-cyan-400">{unreadCount} non lu{unreadCount > 1 ? 'es' : 'e'}</span>
            )}
          </div>
        </div>
      </div>
