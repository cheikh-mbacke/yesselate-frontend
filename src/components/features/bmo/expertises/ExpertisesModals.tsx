 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Expertises par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer amber par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type ExpertisesModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: ExpertisesModalType | null;
  data?: any;
}

interface ExpertisesModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function ExpertisesModals({ modal, onClose }: ExpertisesModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <ExpertisesStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <ExpertisesDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <ExpertisesExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <ExpertisesSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <ExpertisesShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <ExpertisesConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function ExpertisesStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-amber-400" />
...
// Detail Modal
// ================================
function ExpertisesDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-amber-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-amber-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-amber-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-amber-500 hover:bg-amber-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function ExpertisesExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function ExpertisesSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function ExpertisesShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function ExpertisesConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
