...
export default function TicketsClientsPage() {
  ...
  return (
    <div className={cn("h-full flex flex-col bg-gradient-to-br from-slate-50 via-white to-purple-50/20 dark:from-[#0f0f0f] dark:via-[#1a1a1a] dark:to-purple-950/10", fullscreen && "fixed inset-0 z-50")}>
        ...
        <div className="px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="p-2 rounded-xl bg-gradient-to-br from-purple-500 to-purple-600 shadow-lg shadow-purple-500/25"><Ticket className="w-5 h-5 text-white" /></div>
              <div><h1 className="text-xl font-bold">Tickets Clients</h1><p className="text-sm text-slate-500">Support et réclamations clients</p></div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setCommandPaletteOpen(true)} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm text-slate-500 hover:border-purple-500/50 transition-colors">
              ...
              <button onClick={handleRefresh} className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"><RefreshCw className="w-4 h-4 text-slate-500" /></button>
              <button onClick={() => setStatsModalOpen(true)} className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"><BarChart3 className="w-4 h-4 text-slate-500" /></button>
              <button onClick={() => setDirectionPanelOpen(!directionPanelOpen)} className={cn("p-2.5 rounded-xl border transition-colors", directionPanelOpen ? "border-purple-500 bg-purple-500/10 text-purple-600" : "border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-500")}>{directionPanelOpen ? <PanelRightClose className="w-4 h-4" /> : <PanelRight className="w-4 h-4" />}</button>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/50 border border-slate-200/70 dark:border-slate-800 hover:shadow-lg transition-shadow cursor-pointer" onClick={() => onOpenQueue('critical', 'Critiques', '⚡')}>
        ...
        <h3 className="font-semibold mb-2">Tickets Critiques</h3>
        <p className="text-sm text-slate-500">Réclamations urgentes nécessitant action immédiate</p>
      </div>
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/50 border border-slate-200/70 dark:border-slate-800 hover:shadow-lg transition-shadow cursor-pointer" onClick={() => onOpenQueue('open', 'Ouverts', '📬')}>
        <AlertTriangle className="w-8 h-8 text-blue-500 mb-3" />
        <h3 className="font-semibold mb-2">Tickets Ouverts</h3>
        <p className="text-sm text-slate-500">Nouvelles demandes en attente de traitement</p>
      </div>
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/50 border border-slate-200/70 dark:border-slate-800 hover:shadow-lg transition-shadow cursor-pointer" onClick={() => onOpenQueue('pending', 'En attente', '⏳')}>
        ...
        <p className="text-sm text-slate-500">Tickets en attente d'informations client</p>
      </div>
    </div>
  );
}
