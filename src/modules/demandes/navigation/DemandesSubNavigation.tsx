/**
 * Navigation secondaire et tertiaire pour le module Demandes
 * Gère les niveaux 2 (sub-category) et 3 (sub-sub-category)
 * Pas de répétition - tout vient de la config centrale
 */
...
export function DemandesSubNavigation({
  ...
  const mainLabel = demandesNavigationConfig[mainCategory]?.label || 'Demandes';
  ...
  return (
    <div className="bg-slate-900/60 border-b border-slate-700/50">
      ...
      <div className="px-4 py-2 flex items-center gap-2 text-sm border-b border-slate-800/50">
        <span className="text-slate-500">Demandes</span>
      ...
      {/* Sub-Sub Categories (Niveau 3) - Affiché seulement si une sub-category est active */}
      {subSubCategories.length > 0 && subCategory && onSubSubCategoryChange && (
