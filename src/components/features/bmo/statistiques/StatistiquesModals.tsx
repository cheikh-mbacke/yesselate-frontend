 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Statistiques par le nom du module (ex: Delegations, Finances)
...
// ================================
export type StatistiquesModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: StatistiquesModalType | null;
  data?: any;
}

interface StatistiquesModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function StatistiquesModals({ modal, onClose }: StatistiquesModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <StatistiquesStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <StatistiquesDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <StatistiquesExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <StatistiquesSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <StatistiquesShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <StatistiquesConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function StatistiquesStatsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer 
      onClose={onClose} 
      title="Statistiques Statistiques" 
...
// Detail Modal
// ================================
function StatistiquesDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
...
// Export Modal
// ================================
function StatistiquesExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function StatistiquesSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function StatistiquesShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function StatistiquesConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
