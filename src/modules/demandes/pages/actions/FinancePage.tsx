/**
 * Page Actions Finance - Actions prioritaires Finance
 */
...
export function FinancePage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-200 mb-2">Actions prioritaires - Finance</h1>
        <p className="text-slate-400">{demandesData.length} action{demandesData.length > 1 ? 's' : ''} prioritaire{demandesData.length > 1 ? 's' : ''} pour le service Finance</p>
      </div>
      ...
      {demandesData.length === 0 ? (
        ...
        <div className="space-y-3">
          {demandesData.map((demande) => (
            <ActionCard key={demande.id} demande={demande} service="FINANCE" />
          ))}
        </div>
      )}
    </div>
