 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Litiges par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer red par la couleur du module (ex: purple, emerald, blue)
...
interface ModalState {
  isOpen: boolean;
  type: LitigesModalType | null;
  data?: any;
}

interface LitigesModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function LitigesModals({ modal, onClose }: LitigesModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <LitigesStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <LitigesDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <LitigesExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <LitigesSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <LitigesShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <LitigesConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function LitigesStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-red-400" />
...
// Detail Modal
// ================================
function LitigesDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-red-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-red-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-red-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-red-500 hover:bg-red-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function LitigesExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function LitigesSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function LitigesShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function LitigesConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
