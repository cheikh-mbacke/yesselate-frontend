...
export function ValidesPage() {
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <div className="text-slate-400">Chargement des documents validés...</div>
      </div>
    );
  }
    ...
    <div className="p-6 space-y-6">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h2 className="text-lg font-semibold text-slate-200">Documents validés</h2>
        </div>
        <p className="text-sm text-slate-400">
          Historique des documents validés ({documents?.length || 0})
        </p>
      </div>
      ...
      {documents && documents.length > 0 ? (
        ...
        <div className="p-8 rounded-lg border border-slate-700/50 bg-slate-800/30 text-center">
          <div className="text-slate-400">Aucun document validé</div>
        </div>
      )}
