/**
 * Sidebar de navigation pour le module Echanges Structures
...
import { echangesStructuresNavigationConfig, type NavNode } from './echangesStructuresNavigationConfig';
import type { EchangesStructuresMainCategory } from '../types/echangesStructuresNavigationTypes';

interface EchangesStructuresSidebarProps {
...
export function EchangesStructuresSidebar({
  activeCategory,
  activeSubCategory,
  collapsed = false,
  stats = {},
...
}: EchangesStructuresSidebarProps) {
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' | 'success' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'escalade' || node.id === 'critiques' || node.id === 'en_retard') return 'critical';
      if (node.id === 'ouvert' || node.id === 'en_traitement') return 'warning';
    }
    ...
    const handleClick = () => {
      if (hasChildren) {
        ...
        if (level === 0 && node.children && node.children.length > 0) {
          onCategoryChange(node.id as EchangesStructuresMainCategory, node.children[0].id);
        }
      } else {
        if (level === 1) onCategoryChange(activeCategory, node.id);
        else onCategoryChange(node.id as EchangesStructuresMainCategory);
      }
    };
    ...
    >
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Échanges</span>
          </div>
        )}
      ...
      {!collapsed && (
        <div className="px-3 pb-3 border-b border-slate-700/50">
          <button
            ...
            className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm text-slate-400 bg-slate-800/50 hover:bg-slate-700/50 border border-slate-700/50 transition-colors"
          >
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {Object.values(echangesStructuresNavigationConfig).map((node) => (
