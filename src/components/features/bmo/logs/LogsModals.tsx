 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Logs par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer gray par la couleur du module (ex: purple, emerald, blue)
...
// Main Modal Router
// ================================
export function LogsModals({ modal, onClose }: LogsModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <LogsStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <LogsDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <LogsExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <LogsSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <LogsShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <LogsConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function LogsStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-gray-400" />
...
// Detail Modal
// ================================
function LogsDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-gray-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-gray-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-gray-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-gray-500 hover:bg-gray-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function LogsExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function LogsSettingsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <ModalContainer onClose={onClose} title="Paramètres" icon={<Settings className="w-5 h-5 text-slate-400" />}>
      <div className="space-y-6">
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <input
              ...
              className="w-4 h-4 rounded border-slate-600 text-gray-500 focus:ring-gray-500"
            />
          </div>
          {autoRefresh && (
            <div>
        ...
        <Button onClick={onClose} className="w-full bg-gray-500 hover:bg-gray-600">
          Enregistrer
        </Button>
      </div>
    </ModalContainer>
...
// Shortcuts Modal
// ================================
function LogsShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function LogsConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
