 ...
 * Usage:
 * 1. Copier ce fichier
 * 2. Remplacer Parametres par le nom du module (ex: Delegations, Finances)
 * 3. Remplacer cyan par la couleur du module (ex: purple, emerald, blue)
...
// ================================
export type ParametresModalType =
  | 'stats'
  | 'detail'
  | 'export'
  | 'settings'
...
interface ModalState {
  isOpen: boolean;
  type: ParametresModalType | null;
  data?: any;
}

interface ParametresModalsProps {
  modal: ModalState;
  onClose: () => void;
}
...
// Main Modal Router
// ================================
export function ParametresModals({ modal, onClose }: ParametresModalsProps) {
  if (!modal.isOpen || !modal.type) return null;

  if (modal.type === 'stats') {
    return <ParametresStatsModal onClose={onClose} />;
  }

  if (modal.type === 'detail') {
    return <ParametresDetailModal onClose={onClose} data={modal.data} />;
  }

  if (modal.type === 'export') {
    return <ParametresExportModal onClose={onClose} />;
  }

  if (modal.type === 'settings') {
    return <ParametresSettingsModal onClose={onClose} />;
  }

  if (modal.type === 'shortcuts') {
    return <ParametresShortcutsModal onClose={onClose} />;
  }

  if (modal.type === 'confirm') {
    return <ParametresConfirmModal onClose={onClose} data={modal.data} />;
  }
...
// Stats Modal
// ================================
function ParametresStatsModal({ onClose }: { onClose: () => void }) {
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatCard label="Actifs" value={stats.active} color="text-cyan-400" />
...
// Detail Modal
// ================================
function ParametresDetailModal({ onClose, data }: { onClose: () => void; data?: any }) {
  ...
  if (loading) {
    return (
      <ModalContainer onClose={onClose} title="Détails" icon={<FileText className="w-5 h-5 text-cyan-400" />} size="large">
        <div className="flex items-center justify-center py-12">
          <Loader2 className="w-8 h-8 animate-spin text-cyan-400" />
        </div>
      </ModalContainer>
  ...
  return (
    <ModalContainer onClose={onClose} title={item.title} icon={<FileText className="w-5 h-5 text-cyan-400" />} size="large">
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
        ...
        <div className="flex gap-3 pt-4 border-t border-slate-700/50">
          <Button className="flex-1 bg-cyan-500 hover:bg-cyan-600" onClick={onClose}>
            Fermer
          </Button>
        </div>
      </div>
...
// Export Modal
// ================================
function ParametresExportModal({ onClose }: { onClose: () => void }) {
...
// Settings Modal
// ================================
function ParametresSettingsModal({ onClose }: { onClose: () => void }) {
...
// Shortcuts Modal
// ================================
function ParametresShortcutsModal({ onClose }: { onClose: () => void }) {
  const shortcuts = [
...
// Confirm Modal
// ================================
function ParametresConfirmModal({ onClose, data }: { onClose: () => void; data?: any }) {
