/**
 * Sidebar de navigation principale pour Validation Paiements
 * Architecture inspirée d'Analytics et Gouvernance
 */
...
export const PaiementsCommandSidebar = React.memo(function PaiementsCommandSidebar({
  ...
  const paiementsCategories: SidebarCategory[] = [
    ...
    { 
      ...
      label: 'Urgents', 
      icon: AlertTriangle, 
      badge: stats?.byUrgency?.critical || 0,
      badgeType: 'critical'
    },
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Validation Paiements</span>
          </div>
        )}
        {collapsed && (
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {paiementsCategories.map((category) => {
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isActive ? 'text-emerald-400 scale-110' : 'text-slate-400 group-hover:text-slate-200 group-hover:scale-105'
                  )}
                >
