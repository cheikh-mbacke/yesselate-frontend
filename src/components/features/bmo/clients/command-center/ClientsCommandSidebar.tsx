/**
 * Sidebar de navigation principale pour Clients
...
const clientsCategories: SidebarCategory[] = [
  { id: 'overview', label: "Vue d'ensemble", icon: LayoutDashboard },
  { id: 'prospects', label: 'Prospects', icon: UserPlus, badge: 12 },
  { id: 'premium', label: 'Premium', icon: Crown, badge: 8 },
  { id: 'litiges', label: 'Litiges', icon: AlertTriangle, badge: 3, badgeType: 'critical' },
  { id: 'historique', label: 'Historique', icon: History },
  { id: 'contrats', label: 'Contrats', icon: FileText, badge: 24 },
  { id: 'performance', label: 'Performance', icon: TrendingUp },
  { id: 'entreprises', label: 'Entreprises', icon: Building2, badge: 156 },
  { id: 'interactions', label: 'Interactions', icon: MessageSquare, badge: 5, badgeType: 'warning' },
];
...
export const ClientsCommandSidebar = React.memo(function ClientsCommandSidebar({
  activeCategory,
  collapsed,
  onCategoryChange,
  onToggleCollapse,
  onOpenCommandPalette,
}: ClientsCommandSidebarProps) {
  return (
    <aside
      className={cn(
    ...
    >
      ...
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && (
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-200 text-sm">Clients</span>
          </div>
        )}
        {collapsed && (
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {clientsCategories.map((category) => {
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  'group relative',
                  isActive
                    ? 'bg-cyan-500/10 border border-cyan-500/30 scale-[1.02]'
              ...
              >
                ...
                <div
                  className={cn(
                    ...
                    isActive ? 'text-cyan-400 scale-110' : 'text-slate-400 group-hover:text-slate-200 group-hover:scale-105'
                  )}
                >
