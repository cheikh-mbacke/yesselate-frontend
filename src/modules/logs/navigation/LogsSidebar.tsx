/**
 * Sidebar de navigation pour le module Logs
...
import { logsNavigationConfig, type NavNode } from './logsNavigationConfig';
import type { LogsMainCategory } from '../types/logsNavigationTypes';

interface LogsSidebarProps {
  activeCategory: LogsMainCategory;
...
export function LogsSidebar({
  ...
  const getBadgeTypeForNode = useCallback((node: NavNode): 'default' | 'warning' | 'critical' | 'success' => {
    ...
    if (badge && typeof badge === 'number' && badge > 0) {
      if (node.id === 'errors' || node.id === 'security') return 'critical';
      if (node.id === 'warnings') return 'warning';
    }
    ...
    const handleClick = () => {
      if (hasChildren) {
        ...
        if (level === 0 && node.children && node.children.length > 0) {
          onCategoryChange(node.id as LogsMainCategory, node.children[0].id);
        }
      } else {
        if (level === 1) onCategoryChange(activeCategory, node.id);
        else onCategoryChange(node.id as LogsMainCategory);
      }
    };
  ...
  return (
    <aside className={cn('flex flex-col border-r border-slate-700/50 bg-slate-900/80 backdrop-blur-xl transition-all duration-300', 'fixed sm:relative z-40 h-full', collapsed ? 'w-16' : 'w-64')}>
      <div className="flex items-center justify-between p-3 border-b border-slate-700/50">
        {!collapsed && <div className="flex items-center gap-2"><Terminal className="h-5 w-5 text-orange-400" /><span className="font-semibold text-slate-200 text-sm">Logs</span></div>}
        {collapsed && <Terminal className="h-5 w-5 text-orange-400 mx-auto" />}
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        <div className="space-y-1 px-2">
          {Object.values(logsNavigationConfig).map((node) => <NavNodeComponent key={node.id} node={node} level={0} />)}
        </div>
      </nav>
      {!collapsed && <div className="border-t border-slate-700/50 p-3"><div className="text-xs text-slate-500 text-center">Logs v2.0</div></div>}
    </aside>
  );
}
